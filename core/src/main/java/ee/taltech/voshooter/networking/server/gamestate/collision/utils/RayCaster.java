...
public class RayCaster {
    ...
    private Fixture lastCollision;

    public Body getFirstCollision(World world, Vector2 initialPos, Vector2 rayDirection, float maxDistance, HashSet<Body> excluded) {
        lastCollision = null;

        Vector2 endPos = initialPos.cpy().add(rayDirection.cpy().setLength(maxDistance));
        world.rayCast(new CallBack(), initialPos, endPos);

        return (lastCollision == null) ? null : lastCollision.getBody();
    }
